...
import com.shigoo.cashregister.adapters.viewHolder.GiveTableHoldView;
 ...
 */
public class GiveTableListAdapter extends BaseQuickAdapter<Table, GiveTableHoldView> implements Filterable {
    private List<Table> backData;
    private Filter mFilter;
    private int CurrentPosition = -1;

    public GiveTableListAdapter(int layoutResId, @Nullable List<Table> data) {
        super(layoutResId, data);
        this.backData = data;
    }

    @Override
    protected void convert(GiveTableHoldView helper, Table item) {
        helper.setItem(item);
        if (CurrentPosition < getData().size()) {
            helper.setSelect(CurrentPosition);
        }
    }
